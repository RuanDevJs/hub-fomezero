...
import { IDonation, TypeDonationPayload } from "@/types/Donation";
...
export default class DonationRepository {
  ...
  async findAll() {
    ...
    try {
      if (client && database) {
        return await (await database.collection<IDonation>("newDonation").find().toArray()).map(oldValue => ({ ...oldValue, _id: oldValue._id.toString() }));
      }
    } catch (error) {
  ...
  async save(payload: TypeDonationPayload) {
    ...
    try {
      if (client && database) {
        const newDonation = await database.collection<TypeDonationPayload>("donations").insertOne({ ...payload, created_at: new Date(), status: "INICIADA" });
